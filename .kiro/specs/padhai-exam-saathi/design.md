...
7. Enforce a timeout budget bounded by API Gateway's hard ceiling: HTTP APIs (API Gateway v2) cap integration timeouts at 30 seconds and this **cannot be raised** (confirmed against AWS's documented quotas - the increase path that exists for REST APIs does not apply to HTTP APIs). The Lambda's own timeout is set to 29s (the practical max before the Gateway cuts the connection regardless), and the Bedrock client's `read_timeout=24` leaves ~5s of that budget for S3 write/delete, JSON parsing, and cold-start overhead. **Revised after a real production incident**: the original tuning (`read_timeout=15`, Lambda timeout 20s) was validated only against a toy 4-word text prompt (~1.4s) and a slightly longer 300-word text prompt (~10.4s) - both misleadingly fast. A real handwritten-notes photo took 15-17s end to end in production, consistently exceeding the 15s Bedrock timeout and surfacing as a 504 to the student. Also reduced `maxTokens` from 2000 to 1200 (still comfortably above the ~900-token ceiling implied by the Req 2.2 word limits) since generation time scales with the token budget the model is allowed to use, and 2000 gave more headroom than needed while directly adding to worst-case latency. Verified against the actual failing case: a real 48KB JPEG handwritten-notes photo (converted from the reported input) completed in 14.4s after this fix, previously timing out at the old 15s ceiling.
